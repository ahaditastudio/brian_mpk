...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!imageFile || !description || !videoCount) {
    ...
    try {
      ...
      formData.append('Produk', imageFile);
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl max-w-4xl w-full border border-slate-200">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 max-w-2xl mx-auto">
            Platform otomatis untuk memproduksi konten video secara massal. Cukup unggah gambar, tulis deskripsi, dan biarkan sistem kami yang bekerja.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-8 space-y-6 md:space-y-0">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                        Gambar Referensi <span className="text-red-500">*</span>
                    </label>
                    <div className="mt-1">
                        <label htmlFor="image-upload" className="relative flex justify-center items-center w-full h-64 px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors">
                            {imagePreview ? (
